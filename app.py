...
def mega_login_handler(message, bot):
    try:
        credentials = message.text.split(maxsplit=1)[1]
        email, password = credentials.split()
        global mega_session
        mega_session = mega_client.login(email, password)
        bot.reply_to(message, "Mega.nz login successful.")
    except Exception as e:
        logging.error(f"Mega.nz login failed: {e}")
        bot.reply_to(message, "Failed to log in to Mega.nz. Ensure the format is `/meganz email password`.")

def mega_download_handler(message, bot):
    if not mega_session:
        bot.reply_to(message, "Please log in to Mega.nz first using `/meganz email password`.")
        return

    try:
        link = message.text.split(maxsplit=1)[1]
        file = mega_session.download_url(link, dest_path=output_dir)
        bot.reply_to(message, f"Download completed: {file}")
        with open(file, 'rb') as f:
            bot.send_document(message.chat.id, f)
        os.remove(file)
    except Exception as e:
        logging.error(f"Mega.nz download failed: {e}")
        bot.reply_to(message, f"Failed to download file from Mega.nz. Error: {str(e)}")

# Telegram command handlers
@bot2.message_handler(commands=['meganz'])
def handle_mega_login(message):
    mega_login_handler(message, bot2)

@bot2.message_handler(commands=['mega'])
def handle_mega_download(message):
    mega_download_handler(message, bot2)

# Telegram general message handler
